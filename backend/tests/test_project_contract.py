...
def test_project_metadata():
    assert PROJECT_TYPE == 'LangChain tool-calling agent'
    assert PROJECT_SUBJECT == 'CSV Business Analyst with LangChain Tools, FastAPI, React, PostgreSQL, Docker'
...
def test_pipeline_returns_answer_sources_and_steps():
    ...
    assert result.steps
    assert result.project_type == 'LangChain tool-calling agent'
